...
class Extrae_Bi():
    ...
    def consulta_sql_out(self,nmProcedure_out,IdtReporteIni,IdtReporteFin,nmReporte):
        with StaticPage.conout.connect() as connectionout:
            cursorout = connectionout.execution_options(isolation_level="READ COMMITTED")
            sqlout=text(f"CALL {str(nmProcedure_out)}('{str(IdtReporteIni)}','{str(IdtReporteFin)}','{str(nmReporte)}');")
    ...
    def procedimiento_a_sql(self,IdtReporteIni,IdtReporteFin,nmReporte,nmProcedure_in,nmProcedure_out,txTabla):
        # Iniciamos las conexiones
        EsVacio=True
        if nmReporte == 'update_cubo_bi' or nmReporte == 'borra_impactos_bi' or nmReporte == 'impactos_bi' :
            try:
        ...
        if EsVacio==False and StaticPage.resultado_out.empty == False :
            ...
            self.consulta_sql_bi(IdtReporteIni=IdtReporteIni,IdtReporteFin=IdtReporteFin)
            logging.info(f"Se ha realizado el proceso de {txTabla}")
